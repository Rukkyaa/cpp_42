...
/*   Updated: 2023/03/14 14:41:28 by axlamber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap( void ) :
	_name("Nameless"),
	_hitPoint(CLAP_HIT_POINT),
	_energyPoint(CLAP_ENERGY_POINT),
...
void	ClapTrap::takeDamage( unsigned int amount ) {
	if (_hitPoint > 0) {
		std::cout << BOLD_YELLOW"[Clap trap] OOUCH ! " YELLOW << _name << " took " << amount << " damages " << std::endl;
		if (_hitPoint - (int)amount < 0)
			_hitPoint = 0;
		else
			_hitPoint -= amount;
	} else {
		std::cout << BOLD_RED"[Clap trap] WTF ? " RED << " do you really want to kick a dead ?!" << std::endl;
	}
}
...
void	ClapTrap::beRepaired( unsigned int amount) {
	if (_energyPoint) {
		if (_hitPoint != _maxHitPoint) {
			std::cout << BOLD_MAGENTA"[Clap trap] Wow ! " MAGENTA << _name << " regains " << (_hitPoint + (int)amount > _maxHitPoint ? _maxHitPoint - _hitPoint : amount) << " life points !" << std::endl;
			...
			std::cout << BOLD_RED"[Clap trap] ERROR " RED << _name << " is at the max of his capacity !" << std::endl;
		}
	} else
		std::cout << BOLD_RED"[Clap trap] ERROR " RED << _name << " doesn't have enough energy points to be repaired ! " << std::endl;
}
